...
for (e in 1:3){
  for (i in 1:16){
    dataFinal[i,e,,] =  dataFinal[i,e,,]/boxplot(c(dataFinal[i,e,,]), na.rm=T)$stats[5,]
  }
}



dataFinal = round(dataFinal, digits=5)
print(dataFinal)
...
active_mu = round(active_mu, digits=5)
inactive_mu = round(inactive_mu, digits=5)
...
save(delta, file="delta_ddepn14.Rdata")
save(active_mu, file="active_mu_ddepn14.Rdata")
save(active_sd, file="active_sd_ddepn14.Rdata")
save(inactive_mu, file="inactive_mu_ddepn14.Rdata")
save(inactive_sd, file="inactive_sd_ddepn14.Rdata")

save(dataFinal, file="dataFinal14.Rdata")
