...
loocv_dyn_ddepn <-function(function_,lambda2_,times,obs,n,b,K,delta,lambda,annot,annot_node,T_=NULL,previousNet=NULL,baseline_=NULL,previousBaseline=NULL,active_mu,active_sd,inactive_mu,inactive_sd,prior=NULL,sourceNode=NULL,sinkNode=NULL,allint=FALSE,allpos=FALSE)
{
  ...
  edges_all <- sq_err <- baseline_all <- vector()
	...
	for(x in 1:dim(looc)[1])
	{
		...
		if(!is.na(ele)){
			...
			for(i in 1:times){
				## calculate mean squared error of predicted and observed
				predict <- calcPredictionLOOCV_dyn_ddepn(rem_t,kds,adja,obs_modified,delta,rem_kd,rem_gene,active_mu,active_sd,inactive_mu,inactive_sd)
				sq_err_tmp <- c(sq_err_tmp,((predict-ele)^2))
			}
			res <- NA
			## calculate statistics on learned edges
...
loocv_dyn_sahin03<-function(function_,lambda2_,times,obs,n,b,K,delta,lambda,annot,annot_node,T_=NULL,previousNet=NULL,baseline_=NULL,previousBaseline=NULL,active_mu,active_sd,inactive_mu,inactive_sd,prior=NULL,sourceNode=NULL,sinkNode=NULL,allint=FALSE,allpos=FALSE)
{
  ...
  edges_all <- sq_err <- baseline_all <- vector()
	...
	for(x in 1:dim(looc)[1])
	{
		...
		if(!is.na(ele)){
			...
			for(i in 1:times){
				## calculate mean squared error of predicted and observed
				predict <- calcPredictionLOOCV_dyn_sahin03(rem_t,kds,adja,obs_modified,delta,rem_kd,rem_gene,active_mu,active_sd,inactive_mu,inactive_sd)
				sq_err_tmp <- c(sq_err_tmp,((predict-ele)^2))
			}
			res <- NA
			## calculate statistics on learned edges
