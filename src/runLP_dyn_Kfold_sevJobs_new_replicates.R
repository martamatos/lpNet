runLP_dyn_Kfold_sevJobs_new_replicates = function(LPfunction, CVfunction, predFunction,getAdja_function, getBaseline_function, kfold, geneState_, sd_all, totalruns, loocv_times, replnum, b_, n, K, T_, T_undNet, annot, annot_node, active_mu, inactive_mu, active_sd, inactive_sd, stepsize,outputDir,prior=NULL,startNode=NULL,endNode=NULL,allint=FALSE,allpos=FALSE,mu_type=mu_type,delta_type=delta_type)
{
...
#		aucROC[[sd_i]] <- aucPR[[sd_i]] <- vector()
		...
		for(i in 1:totalruns)
		{
			...
			obs[[sd_i]] = array(NA, c(n,K,T_, replnum))
			tmp = c()
