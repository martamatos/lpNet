...
printAUC_median_undMSE = function(auc,sd_all,randROC,totalruns, ouputDir)
{
		auc2 = vector()
		auc2 = rbind(auc2, auc)
		auc2 = rbind(auc2, c(randROC,NA,NA,NA,NA,NA,NA,NA,NA,NA))
...
#		print(auc2)
		write.table(auc2,sprintf('%s/auc_und_medianMSE.txt', ouputDir),quote=FALSE,row.names=F)
