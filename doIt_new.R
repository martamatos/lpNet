...
#
doIt_new <- function(LPfunction,CVfunction, loocv_times, kfold=NULL,stepsize,obs,n,b,K,delta,annot,annot_node,T_=NULL,prior=NULL,startNode=NULL,endNode=NULL,allint=FALSE,allpos=FALSE,active_mu,active_sd,inactive_mu,inactive_sd,muPgene=FALSE,muPgk=FALSE,muPgt=FALSE,muPgkt=FALSE,deltaPk=FALSE,deltaPt=FALSE,deltaPkt=FALSE)
{

	lambda <- calcRangeLambda_new(delta,obs,stepsize=stepsize,deltaPk=deltaPk)
	MSE <- Inf
  print("lambda")
  print(lambda)
	for(lamd in lambda)
	{
		res <- CVfunction(function_=LPfunction,kfold=kfold,times=loocv_times,obs=obs,n=n,b=b,K=K,delta=delta,lambda=lamd,annot=annot,annot_node=annot_node,T_=T_,active_mu=active_mu,active_sd=active_sd,inactive_mu=inactive_mu,inactive_sd=inactive_sd,prior=prior,sourceNode=sourceNode,sinkNode=sinkNode,allint=allint,allpos=allpos,muPgene=muPgene,muPgk=muPgk,muPgt=muPgt,muPgkt=muPgkt,deltaPk=deltaPk,deltaPt=deltaPt,deltaPkt=deltaPkt)
...
# returns the edges calculated using the best lambda
#
doIt_dyn_new <- function(LPfunction,CVfunction, loocv_times, kfold=NULL,stepsize,obs,n,b,K,delta,annot,annot_node,T_=NULL,prior=NULL,startNode=NULL,endNode=NULL,allint=FALSE,allpos=FALSE,active_mu,active_sd,inactive_mu,inactive_sd,muPgene=FALSE,muPgk=FALSE,muPgt=FALSE,muPgkt=FALSE,deltaPk=FALSE,deltaPt=FALSE,deltaPkt=FALSE)
{

	lambda <- calcRangeLambda_dyn_new(delta,obs,stepsize=stepsize,deltaPk=deltaPk,deltaPt=deltaPt,deltaPkt=deltaPkt)
	MSE <- Inf
  print("lambda")
  print(lambda)
	for(lamd in lambda)
