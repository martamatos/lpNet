runLP_dyn_Kfold02 = function(function_, lambda2_, kfold, geneState_, sd_all, totalruns, loocv_times, replnum, b_, n, K, T_, T_nw_,T_undNet, annot, annot_node, active_mu, inactive_mu, active_sd, inactive_sd,graphSize_,outputDir)
{
...
#--------------------------------------------------------
	...
	for(std in sd_all)
	{
		...
		for(i in 1:totalruns)
		{
			...
			ret <- doIt_dyn_kfold(function_,kfold,loocv_times,obs,n,b,K,delta,lambda=lamd,lambda2_,annot,annot_node,T_,previousNet,baseline,previousBaseline,prior=NULL,startNode=NULL,endNode=NULL,allint=FALSE,active_mu,active_sd,inactive_mu,inactive_sd)
