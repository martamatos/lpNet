doILP_dyn_noSelfAct_fixedBaseline <-
function(obs,delta,lambda,b,n,K,T_=NULL,annot,previousNet=NULL,baseline=NULL,previousBaseline=NULL,prior=NULL,sourceNode=NULL,sinkNode=NULL,all.int=FALSE,all.pos=FALSE)
{
  ...
  if(all.pos) W <- matrix(0,nrow=K*(n+n*n)+n,ncol=n*n+n)
  else W <- matrix(0,nrow=K*(n+n*n)+n,ncol=2*n*n+n)
  colnames(W) <- annot
  ...
  bvec <- rep(0,K*(n+n*n)+n)
  J <- seq(1,n)
  count <- 1
  slack_var <- rep(FALSE,K*(n+n*n)+n) # TRUE if the sign of equation is changed
  ...
  #slack variables=length(b)
	if(lambda!=0){
		...
		for(j in 1:length(f.dir)){
			if( (f.dir[j]==">=" && j <= n) || (f.dir[j]=="=")) xi <- c(xi,0)
			if(f.dir[j]=="<=" && j <= n) xi <- c(xi,-1)
			if(f.dir[j]==">=" && (j > n) && (j <= K*(n*n+n)))  xi <- c(xi,1)
		}
