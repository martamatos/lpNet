doILP_dyn_nonIterative_constrSep_lambda2_obsj_gt <-
  ...
  for (t in 2:T_)
  {
		...
		for(i in (n+1):(n*n+n))
		{
			...
			if (ii != jj){
				for (k in 1:K){
					# positive parameter
					if(!is.na(obs[jj,k,t-1])){
						id <- which(annot==paste("w+",jj,ii,t-1,sep="_"))
						W[count,id] <- W[count,id] - obs[jj,k,t-1]
					}
					if(!is.na(obs[jj,k,t])){
						id <- which(annot==paste("w+",jj,ii,t,sep="_"))
						W[count,id] <- W[count,id] + obs[jj,k,t]
					}
				} # end k
			}
