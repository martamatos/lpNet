doILP_dyn_noSelfAct_equalSign_unconstrained <-
...
#  print(W)
  ...
  lambda2=2
	if(lambda!=0){
		...
		sl <- matrix(0,nrow=dim(W)[1],ncol=dim(W)[1])
		annot_s <- paste("s",seq(dim(W)[1]+1,2*dim(W)[1]),sep="_")
		print("dims")
		print(dim(sl))
		print(length(annot_s))
		colnames(sl) <- annot_s
		# attention: for the constr. where observation is smaller than threshold
		xi <- vector()
		for(j in 1:length(f.dir)){
		...
		if(all.pos){
			cvec <- c(rep(1,n*n),rep(1,n),rep(1/lambda,n),rep(1/lambda2,n*n))
			# self-activation is not allowed
			id_self <- c(which(annot==paste("w+",seq(1,n),seq(1,n),sep="_")))
		}
		else{
			cvec <- c(rep(1,n*n),rep(1,n*n),rep(1,n),rep(1/lambda,n),rep(1/lambda2,n*n))
			# self-activation is not allowed
