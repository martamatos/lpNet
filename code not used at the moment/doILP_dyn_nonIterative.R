...
#	print(f.dir)
	if(lambda!=0){
		...
		annot_s <- paste("s",seq(1,dim(W)[1]*2),sep="_")
		colnames(sl) <- annot_s[1:dim(W)[1]]
		...
		W <- cbind(W,sl)
		sl <- matrix(0,nrow=dim(W)[1],ncol=dim(W)[1])
		colnames(sl) <- annot_s[(dim(W)[1]+1):(2*dim(W)[1])]
  ...
  else {	#build objective functio with no slack variable
		if(all.pos){
